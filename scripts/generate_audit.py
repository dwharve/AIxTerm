...
class AuditGenerator:
    ...
    def get_repository_structure(self, max_depth: int = 4) -> str:
        ...
        try:
            ...
            result = subprocess.run(
                ['tree', '-d', f'-L', str(max_depth), '-I', '|'.join(self.exclude_dirs)],
                cwd=self.repo_root,
                capture_output=True,
                text=True
            )
            ...
            for file in files:
                ...
                try:
                    ...
                    for pattern_name, pattern in config_patterns.items():
                        ...
                        if matches:
                            patterns[pattern_name].append(f"{rel_path} ({len(matches)} occurrences)")
                ...
                if file.endswith(tuple(self.code_extensions)) or file in ['Makefile', 'README.md']:
                    ...
                    try:
                        ...
                        for i, line in enumerate(content.split('\n'), 1):
                            matches = re.finditer(annotation_patterns, line, re.IGNORECASE)
                            for match in matches:
                                annotation_type = match.group(1).upper()
                                comment_text = match.group(2).strip()[:100]  # Limit length
                                annotations.append({
                                    'file': rel_path,
                                    'line': str(i),
                    ...
                    while i < len(lines):
                        ...
                        if (lines[i].strip().startswith('#') or 
                            ...
                            if block_length > 5:  # Only blocks >5 lines
                                ...
                                code_indicators = len(re.findall(r'[=(){}\[\];]', block_text))
    ...
    def find_potential_duplications(self) -> List[str]:
        ...
        for func_name, files in function_names.items():
            if len(files) > 1:
                duplications.append(f"Function '{func_name}' appears in: {', '.join(files)}")
    ...
    def analyze_test_coverage_surface(self) -> Dict[str, Any]:
        ...
        for test_file in coverage['test_files']:
            ...
            if test_name.startswith('test_'):
                ...
                coverage['test_to_source_mapping'].append(f"{test_file} -> {source_name}")
                ...
                try:
                    ...
                    if lines > 500:  # Large file
                        ...
                        if class_count > 5 or func_count > 20:
                            hotspots.append(f"Complex module: {rel_path} ({lines} lines, {class_count} classes, {func_count} functions)")
    ...
    def generate_findings(self, annotations: List, duplications: List, file_metrics: Dict, 
        ...
        for ann_type, count in annotation_counts.items():
            if count > 5:  # Significant number of annotations
                self.add_finding(
                    category="Legacy/Dead" if ann_type in ['DEPRECATED', 'LEGACY'] else "Inconsistency",
...
"""
        ...
        if 'quality-check' in makefile_targets:
            quality_gates.append("quality-check (format-check, lint, type-check, import-check, security-check)")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if start_marker in existing_content and end_marker in existing_content:
                ...
                after = existing_content.split(end_marker)[1] if end_marker in existing_content else ""
